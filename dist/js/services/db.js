...
(function(J) {
    ...
    class DB extends Dexie{constructor(){super(getCurrentVaultName());this.version(1).stores({notes:'id,title,*linksTo,*relatedTo',meta:'key'});}}
    const db=new DB(); 
    ...
    const searchNotes=async(q)=>!q?[]:(await db.notes.filter(n=>n.title.toLowerCase().includes(q.toLowerCase())).toArray()).map(n=>({id:n.id,title:n.title}));
    ...
    J.Services.DB = {
        ...
        getTopology, getFavorites, toggleFavorite, getHomeNoteId, setHomeNoteId, getFontSize, setFontSize,
        getSectionVisibility, setSectionVisibility, getAppTheme, setAppTheme, searchNotes, getAllNotes, importNotes
    };
})(window.Jaroet);
