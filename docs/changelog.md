...
- **Improved Markdown editor**: In the markdown editor you can now select text and add `[[`, `*`, `**` and `***` format syntax to surround the selected text. This formats the selected text as a link, italic, bold or bold italic text. 
...
- **Theme choice**: The choosen theme (dark or light) is now stored in the DB. 
